...
- 需求存在任何不确定、歧义、范围缺口或可产生多种合理解释的地方时，必须先向用户确认并得到明确答复后才能开工。确认前禁止实现代码、编写会固化猜测的测试、修改协议/checkpoint、更新长期合同，或把自行推断写入 `AGENTS.md`、Plan 和文档。
- 涉及玩法数量、生成范围、楼层/区域、位置、权威状态、资源预算、联机协议、持久化兼容、部署影响或用户可见效果时，必须逐项复述理解并确认。例如“建筑里有四种弹药”不得自行扩展为“每层各有四种弹药”。
- Reviewer 必须回看用户原始需求和明确澄清，不能只检查实现、测试、Plan 与 `AGENTS.md` 是否彼此自洽。若长期合同可能源自未经确认的推断，必须作为 Finding 阻止交付。
...
- 苍岬岛始终包含 8 个不规则命名主 POI、8 个野外落区和 20 个隐藏路边院落；禁止退化为固定网格、圆环或象限槽位。主 POI 两两至少相距 420m，且至少 1 个位于地图原点 360m 内；全部 16 个正式落区两两至少相距 300m，在中央 `[-600, 600]²` 每 100m 采样时距最近正式落区不得超过 450m。`max(|x|, |z|) >= 800` 的正式落区最多 6 个，`|x| >= 700 && |z| >= 700` 的角落落区最多 1 个；候选不足时必须继续确定性搜索或显式失败，禁止静默放宽正式落区间距。岛屿医院和弹药库完整 footprint 距地图边界至少 200m、建筑中心距原点不超过 1000m，且不得位于角落带；弹药库优先使用安全的旧仓区建筑，没有安全候选时才能选择其他安全非医院建筑，禁止回退到边缘建筑。
...
- 每张地图必须恰好拥有 1 个权威医院和 1 个与医院不同的权威弹药库。三张地图的医院显示名必须统一为 `医院`，布局、场景 metadata、HUD 和小地图不得使用地图专属医院别名。弹药库无论建筑共有几层，都只能在最底层、现有 250 条全局物资预算之外，为每种枪械弹药提供 1 个固定弹药堆，共 4 堆；上层禁止生成弹药库专属弹药。布局、大逃杀初始化、AI、同步、持久化、HUD 和渲染必须使用显式底层物资索引，禁止从数组尾部或建筑层数推断。
- 三张地图的弹药库必须使用同一六边形权威 footprint，且不得成为 skybridge 端点。墙体、门洞、楼板、楼梯、导航、碰撞、LOS、渲染和底层四堆弹药必须消费同一记录；弹药库专用牌必须使用 `decal.poi.ammo-depot` 并贴合真实底层门洞墙面。三张地图都必须同时保留完整的 5 张品牌标牌、医院标记和弹药库牌。
- 所有建筑（包括一层建筑）都必须拥有内置楼梯间，并且每层恰好有 1 条穿过楼板/屋顶开口的权威坡道。禁止外置脚手架楼梯；移动支撑、战斗/视线、导航、物资避让和渲染只能使用共享的内置坡道记录。
- 每栋建筑都要根据 `mapId + mapSeed + buildingId` 选择 1 个确定性建筑轮廓。建筑本体以矩形为绝对多数，目标比例约为矩形 82%、切角六边形 12%、多边形近圆形 6%；医院和参与高层连桥的建筑保持矩形，弹药库固定为六边形。建筑变化还可使用立面柱、檐口和内缩屋顶机房；禁止通过沿屋顶周边添加实体女儿墙或矮墙制造差异。建筑外墙、楼板、开口、楼梯、移动、战斗/视线、投掷物碰撞、导航、服务端权威逻辑和渲染必须共同使用同一轮廓，禁止只在视觉上做圆形/六边形而权威几何仍是矩形。屋顶保持平坦可达，内部楼梯开口必须畅通。最多约 15% 的普通矩形建筑可以确定性生成 0.5m 高的稀疏金属屋顶围栏；围栏只能由细立柱和横杆组成，必须不可碰撞、不可作为射击/视线遮挡、允许子弹穿透，不得写入权威 `wallSegments`，也不需要因为内部楼梯出口在屋顶中央而额外留边缘缺口。
...
- 目前唯一支持的投掷物是破片手雷。每张地图必须在不变的 240 个基础物资点和 10 个额外医疗点之后，追加恰好 10 个手雷点，每点 2 枚。手雷必须作为背包物品并使用 2.5 秒权威引信；3m 内造成 140 点满伤，之后线性衰减，使无头盔/护甲的 100 HP 角色在权威胶囊距离 5m 处恰好受到致死伤害，并在 10m 处降为 0。爆炸网格和飞散动画覆盖 8m 视觉半径，不创建动态爆炸灯光或实时阴影。固定步长飞行、反弹、引信、爆炸遮挡、衰减伤害、自伤、服务端权威、AI 使用、checkpoint 状态、网络同步和表现必须共同使用同一份权威记录与配置。禁止从图片/模型 metadata 推导手雷规则，也禁止使用 Babylon 物理或渲染 hitbox 处理玩法碰撞。
...
- Bot 在安全区等待阶段必须按可导航距离、剩余等待时间、冲刺速度和确定性错峰预算分批转移；时间充足时继续在当前安全区分散搜刮或巡逻，禁止开局全员立即冲向下一圈。人在当前毒圈外或收缩开始后必须立即持续进圈，并压过战斗、治疗、拾取、retreat、patrol 和 forced relocation。
- 首圈等待阶段存活人数仍超过半数且剩余时间大于 25 秒时，Bot 只主动攻击 45m 内近敌，远敌不触发主动追击；受击方向记忆、近距离自卫和 25 秒后恢复的正常 150m 感知保持不变。三地图完整赛局必须锁定前 120 秒至少 26 人存活，禁止通过弱化毒圈伤害或完整比赛时间掩盖开局扎堆互杀。
- Bot 的 active weapon 断弹时必须依次尝试已装填备用枪、当前枪装填、备用枪装填、兼容弹药和可用地面枪；禁止在两个武器槽都完全失去战斗力后原地等待。恢复物资不得引导 Bot 离开当前安全区，目标失效或不可达时必须重选。
...
- AI 多 pellet 武器必须逐 pellet 保留权威 trace、命中和伤害；允许把 AI 的重复 `shot-traced` 展示事件合并为每发最多 1 条代表性 actor/environment 命中，并在整发全部 miss 时不广播展示轨迹，以降低事件和特效压力，但玩家的展示轨迹不得被压缩。
...
- Reviewer 必须对每个代码变更给出明确的性能影响结论和仍可优化空间，不得只写“测试通过”。涉及场景构建、每帧 update、Bot/房间循环、导航、空间索引、网络 tick、序列化、资源加载、Mesh/material/texture、集合扫描或缓存时，必须检查复杂度、重复工作、分配/GC、批处理、缓存复用、视锥裁剪和启动/稳定阶段差异；发现可能超过 `15%` 的回归风险时必须作为 Finding，要求 Builder 提供 main/head 性能对比证据或修复。
