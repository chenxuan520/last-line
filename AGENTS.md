...
- 每张地图必须恰好拥有 1 个权威医院和 1 个与医院不同的权威弹药库。弹药库无论建筑共有几层，都只能在最底层、现有 250 条全局物资预算之外，为每种枪械弹药提供 1 个固定弹药堆，共 4 堆；上层禁止生成弹药库专属弹药。布局、大逃杀初始化、AI、同步、持久化、HUD 和渲染必须使用显式底层物资索引，禁止从数组尾部或建筑层数推断。
