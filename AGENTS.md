...
- The mixed-region map always contains six named macro regions: one fixed dense town with the sole hospital, one fixed sparse rural area, one fixed mountain forest, and three seed-selected town/rural/forest regions. Region kind, terrain, roads, urban presentation roads, buildings, authoritative trees, rocks, hay, loot, navigation, and presentation must consume the same seeded blueprint; forest tree/rock footprints must bound terrain variation and placement offset rather than center-sample steep slopes, and no code-facing map ID may use Chinese pinyin.
