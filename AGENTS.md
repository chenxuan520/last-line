...
- Persisted match checkpoints are compatible only when their complete recoverable state shape is present, including exactly the configured 50 actors and matching actor record keys and `actor.id` values. Running/finished rooms must retain a record of 2–10 object members with non-null, unique actor IDs, and those IDs must equal the checkpoint's complete `kind: "player"` actor set; no member may point to a bot or share an actor. Missing, null, array, or malformed member containers and entries are incompatible persisted state and must be deleted without throwing. The v5 missing-`mapId` exception applies to an otherwise complete legacy state, never to a missing, null, array, truncated `state`, or incomplete roster; Worker and standalone must delete incompatible room and checkpoint records instead of retrying restoration.
