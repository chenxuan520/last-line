...
- Every building has one deterministic architectural profile for its `mapId + mapSeed + buildingId`. Roof edges, facade piers, cornices, and substantial roof screens are authoritative `MapLayout.wallSegments`, quality-independent, and shared by Movement, combat/LOS, throwable collision, navigation, server authority, and rendering. Keep the internal roof stair opening and one explicit east-side roof escape gap clear; never restore presentation-only rooftop silhouette boxes that look solid but can be crossed.
...
- Single-player debug mode is opt-in through `npm run dev -- --debug=true`. Its invulnerability, property editing, item grants, and UI panel must remain local to `BattleRoyaleSession`; never expose debug state through multiplayer protocol, Worker/standalone services, persisted checkpoints, or normal production builds.
...
- Frag grenades are the only supported throwable. Every map appends exactly 10 supplemental grenade loot points containing two grenades each after the unchanged 240 base and 10 supplemental medical points. Grenades remain backpack items; fixed-step flight, bounce, fuse, explosion obstruction, falloff damage, self-damage, server authority, AI use, checkpoint state, network replication, and presentation consume the same authoritative records and config. Never derive grenade rules from image/model metadata or use Babylon physics/hitboxes.
...
- Every new or changed visible feature must be opened in the production build with the Chrome/Edge MCP browser at volume `0`. The implementation agent must use MCP to capture a screenshot of the affected region, then personally open and inspect that screenshot with the available image-viewing tool before completion; taking a screenshot without viewing it does not count. DOM presence, computed styles, unit tests, console checks, and another agent's description are supporting evidence, never substitutes for the implementation agent viewing the rendered screenshot. Compare the changed element against adjacent elements of the same role and explicitly verify font family, font size, weight, color, spacing, alignment, position, clipping, and overlap; a feature that exists but is visually inconsistent is not complete.
...
- Grenade regressions must cover one-shot command consumption, inventory use, high/low throw input, authoritative sweep/bounce/fuse, radial falloff and obstruction, simultaneous insertion orders, self-damage, Bot safety/cooldown/concurrency, the exact 10-by-2 supplemental distribution, Greyfurnace reachability, checkpoint compatibility, multiplayer visibility, and bounded presentation/effect pools.
...
- Persisted match checkpoints are compatible only at the current checkpoint version and when their complete recoverable state shape is present. This includes exactly the configured 50 actors with matching record keys and `actor.id` values; every canonical initial loot key derived from the checkpoint's explicit `mapId + mapSeed` with matching `loot.id`; valid active-grenade records and a strictly greater next sequence; a reachable safe-zone timeline; and backpack stacks that reference `ITEMS`, use positive integer quantities, and stay within configured `maxStack`. Additional valid dynamic loot records remain allowed. Persisted members must be complete `RoomMemberRecord` objects whose record keys equal `playerId`; identity, display name, admission/reconnect credentials, account/session pairing, booleans, timestamps, connection epoch, and actor ID types are validated before use. Running/finished rooms must retain 2–10 non-null, unique actor IDs equal to the checkpoint's complete `kind: "player"` actor set; no member may point to a bot or share an actor. Missing, null, array, partial, malformed, or older-version state is incompatible and must be deleted without throwing on both Worker and standalone.
