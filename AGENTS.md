...
- Persisted match checkpoints are compatible only when their complete recoverable state shape is present. The v5 missing-`mapId` exception applies to an otherwise complete legacy state, never to a missing, null, array, or truncated `state`; Worker and standalone must delete incompatible room and checkpoint records instead of retrying restoration.
