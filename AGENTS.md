...
- Every map has exactly one authoritative hospital and one different authoritative ammunition depot. The depot contributes one fixed stack for each weapon ammo type outside the existing 250-record global loot budget; layout, battle-royale initialization, AI, replication, persistence, HUD, and rendering must consume its explicit building and loot indexes rather than infer array tails.
