...
- Agent 的思考进度、用户沟通、计划、审查结论、交付报告和 GitHub MR/PR 的标题、正文、评论必须全程使用中文。
- 仓库内所有说明文档和 `.agents/plans/` 下的计划必须使用中文。新增或修改 Markdown 后，提交前必须扫描并清除英文叙述。
- 代码标识、文件路径、命令、协议字段、第三方产品名和无法翻译的上游原文可以保留英文，但周围说明必须使用中文。
- Git 提交标题和正文必须使用中文，并准确说明改动目的、范围或修复的问题；禁止使用 `fix`、`update`、`follow-up`、`wip` 等含糊英文标题，也禁止只写“修复问题”“更新代码”之类无法判断实际内容的笼统信息。代码标识或第三方专名确需保留英文时，周围提交说明仍必须是完整中文。第三方机器人自动生成的历史评论不要求改写；Agent 后续主动发布的内容必须使用中文。
