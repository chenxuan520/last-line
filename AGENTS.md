...
- Persisted match checkpoints are compatible only when their complete recoverable state shape is present, including exactly the configured 50 actors and matching actor record keys and `actor.id` values. Persisted members must be complete `RoomMemberRecord` objects whose record keys equal `playerId`; identity, display name, admission/reconnect credentials, account/session pairing, booleans, timestamps, connection epoch, and actor ID types are validated before use. Running/finished rooms must retain 2–10 non-null, unique actor IDs equal to the checkpoint's complete `kind: "player"` actor set; no member may point to a bot or share an actor. Missing, null, array, partial, or malformed member containers and entries are incompatible persisted state and must be deleted without throwing. The v5 missing-`mapId` exception applies to an otherwise complete legacy state, never to a missing, null, array, truncated `state`, or incomplete roster; Worker and standalone must delete incompatible room and checkpoint records instead of retrying restoration.
...
- Before the associated implementation commit is created, record completed review rounds and finding dispositions in the current plan's `## Review`, and record only facts already known at that time under `## Build`. These records must be included in the same commit as the implementation. The implementation commit is the absolute cutoff for repository plan records: once that commit exists, do not edit the plan for that task at all. Commit hashes, CI results, Pages/Worker deployments, version IDs, smoke results, and every other fact learned at or after commit time must be reported to the user only and must never be written back to the repository. No exception may be inferred from documentation, deployment, cleanup, bookkeeping, or working-tree hygiene requirements.
...
- After the task implementation commit has been created—and especially after it has been pushed—treat the repository as read-only for that task. Do not edit plans, documentation, code, configuration, tests, workflows, or hooks; do not create any follow-up commit; and do not amend, rewrite, or force-push history. Post-commit CI/deployment monitoring and verification are read-only activities. Repository changes may resume only for a genuinely new user-requested change, and that new change must contain its own non-plan deliverable; an attempt to “finish records,” “clean the tree,” or “document deployment” is not a new change.
