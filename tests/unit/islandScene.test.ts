...
describe("IslandScene lifecycle", () => {
  ...
  it("releases scenes and loot marker references across restarts", async () => {
    ...
    for (let restart = 0; restart < 4; restart += 1) {
      ...
      setActorWeaponVisual(bundle.viewWeaponRoot, "grenade.frag");
      expect(viewWeaponMeshes.filter((mesh) => mesh.metadata?.weaponId === "grenade.frag")).toHaveLength(2);
      expect(viewWeaponMeshes.filter((mesh) => mesh.metadata?.weaponId === "grenade.frag")
  ...
  it("builds Greyfurnace City with dense buildings and authoritative skybridges", async () => {
    ...
    expect((hospitalSurfaceBatch?.material as StandardMaterial).diffuseTexture).toBeNull();
    expect(bundle.scene.meshes.length).toBeLessThan(930);
  ...
  it("enables dense realistic town presentation only on high quality", async () => {
    ...
    expect(townVisualBatches.every((mesh) =>
      !mesh.isPickable &&
      !mesh.checkCollisions &&
      Number(mesh.metadata?.sourceCount ?? 0) > 0
    )).toBe(true);
