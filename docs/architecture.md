...
Ground loot is replicated in a 60m horizontal footprint, independent of aircraft altitude. Initial/full state seeds the per-player visible set; subsequent frames send only newly visible, dirty-visible, and newly hidden records, with unavailable tombstones preserving bounded client mesh reuse. Pickup and interaction checks remain three-dimensional and authoritative.
