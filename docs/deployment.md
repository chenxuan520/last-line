...
- Verify all three map choices in production artifacts. Greyfurnace City must show the correct HUD/minimap name, dense industrial blocks, reachable 4–5 story floors, authoritative second-story skybridges, and the same map ID on every multiplayer client. Cinder Mist County must show six named regions, the sole `医院` in `赤钟城区`, dense town streets, sparse rural houses and hay, and mountain forests whose trees and rocks visibly sit on slopes. Quick match must never combine island, town, and mixed requests.
