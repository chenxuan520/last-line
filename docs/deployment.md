...
- 验证三张地图。苍岬岛必须保持 8 个不规则命名主 POI、8 个野外落区和中央覆盖，正式落区不得在边缘/四角扎堆，医院与弹药库完整 footprint 距地图边界至少 200m。灰炉城必须显示正确 HUD/小地图名、高密工业街区、可达四/五层、56 条权威二层连桥，并在所有联机客户端使用同一地图 ID。烬岚郡必须显示 6 个紧凑不规则命名区域和 5 条短连接道路；所有山完整位于地图内；唯一 `医院` 位于 `赤钟城区`；高密城镇、稀疏农村/草垛和树石位于坡地的山林必须区分；城镇区域生成 8 条 X/Z 双方向短权威连桥，农村/森林不生成。快速匹配不得混合 island/town/mixed 请求。
- 验证建筑本体以矩形为绝大多数，约 12% 为切角六边形、约 6% 为多边形近圆形；医院和连桥端点保持矩形，三张地图的弹药库固定为不参与连桥的六边形。特殊形状必须在外墙、楼板、门窗、楼梯、移动、战斗、导航和渲染中一致。禁止实体屋顶护栏；最多约 15% 普通矩形建筑生成 0.5m 稀疏视觉金属围栏，围栏不可碰撞、不遮挡射击/视线、允许子弹穿透。
...
- Checkpoint 版本 14 要求完整 island/town/mixed 状态、由 `mapId + mapSeed` 派生的每条标准初始物资记录（不变的 250 条全局、仅底层 4 条六边形弹药库、10 条手雷）、不超过 2.5 秒剩余引信的合法活动手雷、合法背包 stack 和可达安全区时间线；合法后续 drop/death 可额外保留。版本 13 及更早在 Worker/standalone 恢复时删除，因为它们仍可能包含旧苍岬岛地点/特殊建筑坐标、旧手雷伤害规则、矩形或逐层弹药库事实，以及更早的 mixed/建筑碰撞事实。
- 权威地图/协议发布使用维护顺序：禁用新联机入口、排空/关闭房间、部署并 smoke 协议 15 Worker、部署匹配 Pages、重新启用入口，再分别 smoke 三图房间。
