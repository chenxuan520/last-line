...
- Checkpoint version 6 accepts current island, town, and mixed rooms. The compatibility reader accepts version 5 only for island, town, or legacy states without `mapId`; it rejects version 5 mixed and version 4-or-older states. Verify both the preserved old-map path and the rejected mixed rollback path on Worker and standalone before rollout.
- A map-ID/protocol rollout must use the documented maintenance sequence: disable new multiplayer entry, drain/close rooms, deploy and smoke protocol 7 Worker, deploy matching Pages, re-enable entry, then smoke island, town, and mixed rooms separately.
