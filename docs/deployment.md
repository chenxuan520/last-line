...
- Verify both map choices in production artifacts. Greyfurnace City must show the correct HUD/minimap name, dense industrial blocks, reachable 4–5 story floors, authoritative second-story skybridges, and the same map ID on every multiplayer client. Quick match must never combine island and town requests.
- Verify quality isolation with the baseline scene lighting and image processing unchanged on both maps. Low and medium must not create the high-quality island/town visual batches, high-detail character gear, or high-quality HUD class. High-quality island must show shore foam, road wetness, and POI light details. High-quality Greyfurnace must show transparent facade panes, readable underlying wall textures, road/facade/rooftop/industrial detail, door-derived loading-bay decoration that leaves authoritative openings clear, and correct distinct materials for mixed rooftop, pipe, and skyline batches. The aircraft cockpit must remain opaque and visually independent from town glass.
...
- A map-ID/protocol rollout must use the documented maintenance sequence: disable new multiplayer entry, drain/close rooms, deploy and smoke protocol 6 Worker, deploy matching Pages, re-enable entry, then smoke island and town rooms separately.
