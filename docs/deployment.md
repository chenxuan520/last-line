...
- Checkpoint version 7 requires the complete island, town, or mixed state plus the authoritative grenade schema and a reachable safe-zone timeline. Earlier checkpoint versions are deleted on Worker and standalone restore because the independent protocol-7 map and grenade histories cannot be distinguished safely.
- A map-ID/protocol rollout must use the documented maintenance sequence: disable new multiplayer entry, drain/close rooms, deploy and smoke protocol 8 Worker, deploy matching Pages, re-enable entry, then smoke island, town, and mixed rooms separately.
