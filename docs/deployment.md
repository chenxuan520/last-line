...
- Checkpoint version 7 accepts current island, town, and mixed rooms only when every canonical initial loot record derived from the persisted `mapId + mapSeed` is present. This roster contains the unchanged 250 global records plus four ammunition-depot records per interior story; valid later drop/death records may also remain. Version 6 and older rooms use the previous authoritative stair/floor geometry and 250-record initial loot roster, so Worker and standalone must close and delete them instead of restoring.
- An authoritative-map/protocol rollout must use the documented maintenance sequence: disable new multiplayer entry, drain/close rooms, deploy and smoke protocol 8 Worker, deploy matching Pages, re-enable entry, then smoke island, town, and mixed rooms separately.
